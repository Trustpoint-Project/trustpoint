...
from web_ui_automation.models import WebUiAutomationProfileDefinition

from management.models import AuditLog

_DEFAULTS_DIR = Path(__file__).resolve().parent.parent.parent / 'default_profiles'

logger = logging.getLogger(__name__)


class Command(BaseCommand):
    """Creates default Web UI automation profile definitions."""

    help = 'Creates default Web UI automation profile definitions if they do not already exist.'

    def handle(self, *_args: tuple[str], **_kwargs: dict[str, str]) -> None:
        """Load each *.json file from the default/ directory and create profile definitions."""
        logger.info('Starting creation of default Web UI automation profile definitions from %s', _DEFAULTS_DIR)
        ...
        for json_file in json_files:
            try:
                ...
                if not profile_name:
                    logger.error('Web UI automation profile definition in %s is missing required "name" field', json_file)
                    self.stdout.write(self.style.ERROR(f'ERROR: {json_file.name} is missing "name" field'))
                    continue

                profile, created = WebUiAutomationProfileDefinition.objects.get_or_create(
                    name=profile_name,
                    defaults={
                        'profile': wf_data,
                    },
                ...
                if created:
                    created_count += 1
                    logger.info('Created Web UI automation profile definition: %s (from %s)', profile_name, json_file.name)
                    ...
                    AuditLog.create_entry(
                        ...
                        target=profile,
                        target_display=f'Web UI Automation Profile Definition: {profile_name}',
                        actor=None,  # System action
                        details={
                            'source': 'create_default_web_ui_automation_profile_definitions',
                            'source_file': json_file.name,
                        },
                    )
                else:
                    existing_count += 1
                    logger.debug('Web UI automation profile definition already exists: %s', profile_name)
        ...
        logger.info(
            'Finished creating default web UI automation profile definitions. Created: %d, Already existed: %d, Total processed: %d',
            created_count,
            existing_count,
            len(json_files),
        )
