...
class TestAokiInitializationRequestView:
    ...
    def test_get_no_client_cert(self, request_factory, mock_tls_cert):
        ...
        with patch.object(ActiveTrustpointTlsServerCredentialModel.objects, 'first', return_value=mock_tls_cert):
            with patch(
                'aoki.views.NginxTLSClientCertExtractor.get_client_cert_as_x509',
    ...
    def test_get_idevid_authentication_failed(self, request_factory, mock_tls_cert, mock_idevid_cert):
        ...
        with patch.object(ActiveTrustpointTlsServerCredentialModel.objects, 'first', return_value=mock_tls_cert):
            with patch(
                'aoki.views.NginxTLSClientCertExtractor.get_client_cert_as_x509', return_value=(mock_idevid_cert, [])
            ):
                with patch(
                    'aoki.views.IDevIDAuthenticator.authenticate_idevid_from_x509_no_device',
        ...
        with patch.object(ActiveTrustpointTlsServerCredentialModel.objects, 'first', return_value=mock_tls_cert):
            with patch(
                ...
                with patch(
                    ...
                    with patch.object(
                        AokiInitializationRequestView, 'get_owner_credential', return_value=mock_owner_credential
                    ):
                        request = request_factory.get('/aoki/init/')
                        view = AokiInitializationRequestView()
    ...
    def test_get_unsupported_key_type(
        self, request_factory, mock_tls_cert, mock_idevid_cert, domain_instance, mock_owner_credential
    ):
