...
def test_issue_domain_credential(test_device):
    """Test that issuing a domain credential works without mocks."""
    issuer = LocalDomainCredentialIssuer(device=test_device, domain=test_device.domain)
    ...
    assert test_device.onboarding_status == DeviceModel.OnboardingStatus.ONBOARDED, \
