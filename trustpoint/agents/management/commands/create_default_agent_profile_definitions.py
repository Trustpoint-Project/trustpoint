...
class Command(BaseCommand):
    ...
    def handle(self, *_args: tuple[str], **_kwargs: dict[str, str]) -> None:
        ...
        for json_file in json_files:
            try:
                wf_data: dict[str, Any] = json.loads(json_file.read_text(encoding='utf-8'))
                ...
                logger.exception('Missing required field in %s', json_file)
        ...
        logger.info(
            'Finished creating default agent profile definitions. '
            'Created: %d, Already existed: %d, Total processed: %d',
            created_count,
            existing_count,
            len(json_files),
        )
