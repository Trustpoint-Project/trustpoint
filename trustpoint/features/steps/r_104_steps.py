...
def step_truststore_exists(context: runner.Context, truststore_name: str, intended_usage: str) -> None:  # noqa: ARG001
    ...
    with open(truststore_file_path, 'rb') as f:
        ...
        context.response = context.authenticated_client.post(
            '/pki/truststores/add/', truststore_add_form_data, follow=True
        )
...
def step_delete_truststore(context: runner.Context, name: str) -> None:  # noqa: ARG001
    ...
    context.response = context.authenticated_client.get(
        '/pki/truststores/delete/' + str(context.truststore.id), follow=True, HTTP_X_REQUESTED_WITH='XMLHttpRequest'
    )
    ...
    context.response = context.authenticated_client.post(
        f'/pki/truststores/delete/{context.truststore.id}/', data={}, follow=True
    )
    assert context.response.status_code == 200, 'Truststore deletion response'
    assert not TruststoreModel.objects.filter(id=context.truststore.id).exists(), (
        f'Deletion of Truststore with name {name} failed'
    )
