...
class Command(CertificateCreationCommandMixin, BaseCommand, LoggerMixin):
    ...
    def handle(self, *_args: tuple[str], **_kwargs: dict[str, str]) -> None:
        ...
        if created:
            ...
            self.log_and_stdout(
                f'Using existing SecurityConfig with security mode: {security_config.get_security_mode_display()}'
            )
        ...
        rsa2_root_crl = self.generate_empty_crl(
            rsa2_root, rsa2_root_ca_key,
            hashes.SHA256(),
            crl_validity_hours=root_validity_days * 24
        )
        ...
        rsa2_int_ca_crl = self.generate_empty_crl(
            ...
            rsa2_int_ca_key_1,
            hashes.SHA256(),
            crl_validity_hours=intermediate_validity_days * 24
        )
