...
"""Runserver_plus variant that stores the TLS server credential in the database."""
...
class Command(RunServerPlusCommand):
    ...
    def store_tls_certificate(
        self, cert_file_path: str | Path, key_file_path: str | Path
        ...
        if not cert_path.exists() or not key_path.exists():
            self.stdout.write(
                self.style.WARNING(f'Certificate or key file not found: {cert_path}, {key_path}')
            )
        ...
        self.stdout.write(self.style.SUCCESS('Updated ActiveTrustpointTlsServerCredentialModel.'))
