...
class SecurityManagerTest(TestCase):
    ...
    def test_get_security_level_returns_current_level(self):
        ...
        self.assertEqual(self.manager.get_security_level(), SecurityConfig.SecurityModeChoices.BROWNFIELD)
    ...
    def test_is_feature_allowed_returns_true_for_dev_mode(self):
        """Test is_feature_allowed always returns True in LAB mode."""
        self.security_config.security_mode = SecurityConfig.SecurityModeChoices.LAB
        self.security_config.save()

        # Any feature should be allowed in LAB mode
    ...
    def test_is_feature_allowed_with_feature_class(self):
        ...
        # AutoGenPkiFeature is in BROWNFIELD_FEATURES
        self.assertTrue(self.manager.is_feature_allowed(AutoGenPkiFeature))
    ...
    def test_is_feature_allowed_returns_false_when_not_allowed(self):
        ...
        # AutoGenPkiFeature is not in CRITICAL_FEATURES
    ...
    def test_is_feature_allowed_with_target_level_parameter(self):
        ...
        # Current mode is BROWNFIELD, but we check against CRITICAL
        self.security_config.security_mode = SecurityConfig.SecurityModeChoices.BROWNFIELD
        self.security_config.save()

        # AutoGenPkiFeature not allowed at CRITICAL level
        result = self.manager.is_feature_allowed(
            AutoGenPkiFeature, target_level=SecurityConfig.SecurityModeChoices.CRITICAL
        )
        self.assertFalse(result)

        # AutoGenPkiFeature allowed at BROWNFIELD level
        result = self.manager.is_feature_allowed(
            AutoGenPkiFeature, target_level=SecurityConfig.SecurityModeChoices.BROWNFIELD
        )
        self.assertTrue(result)

    def test_is_feature_allowed_with_target_level_dev(self):
        """Test is_feature_allowed with target_level=LAB returns True."""
        self.security_config.security_mode = SecurityConfig.SecurityModeChoices.CRITICAL
        self.security_config.save()

        # Even though current mode is CRITICAL, checking against LAB should return True
        result = self.manager.is_feature_allowed(
            AutoGenPkiFeature, target_level=SecurityConfig.SecurityModeChoices.LAB
        )
        self.assertTrue(result)
    ...
    def test_get_features_to_disable_returns_correct_features(self):
        ...
        # AutoGenPkiFeature is in BROWNFIELD but not in CRITICAL
        features_to_disable = SecurityManager.get_features_to_disable(SecurityConfig.SecurityModeChoices.CRITICAL)

        self.assertIn(AutoGenPkiFeature, features_to_disable)

    def test_get_features_to_disable_returns_empty_for_dev(self):
        """Test get_features_to_disable returns empty list for LAB mode."""
        features_to_disable = SecurityManager.get_features_to_disable(SecurityConfig.SecurityModeChoices.LAB)

        # No features should be disabled in LAB mode
        # (LAB_FEATURES - LAB_FEATURES = empty set)
    ...
    def test_get_features_to_disable_with_different_levels(self):
        ...
        # BROWNFIELD should have fewer features to disable than CRITICAL
        low_disabled = SecurityManager.get_features_to_disable(SecurityConfig.SecurityModeChoices.BROWNFIELD)
        highest_disabled = SecurityManager.get_features_to_disable(SecurityConfig.SecurityModeChoices.CRITICAL)

        # CRITICAL should have more (or equal) features to disable
    ...
    def test_reset_settings_disables_features(self, mock_disable):
        ...
        # Set to BROWNFIELD (AutoGenPkiFeature enabled), then reset to CRITICAL (AutoGenPkiFeature disabled)
        self.security_config.security_mode = SecurityConfig.SecurityModeChoices.BROWNFIELD
        self.security_config.save()

        self.manager.reset_settings(SecurityConfig.SecurityModeChoices.CRITICAL)
    ...
    def test_reset_settings_does_not_disable_allowed_features(self, mock_disable):
        ...
        # Set to CRITICAL, then reset to BROWNFIELD (AutoGenPkiFeature now allowed)
        self.security_config.security_mode = SecurityConfig.SecurityModeChoices.CRITICAL
        self.security_config.save()

        self.manager.reset_settings(SecurityConfig.SecurityModeChoices.BROWNFIELD)
    ...
    def test_enable_feature_does_not_call_enable_when_not_allowed(self, mock_enable):
        ...
        # AutoGenPkiFeature not allowed at CRITICAL
    ...
    def test_is_feature_allowed_handles_string_security_level(self):
        ...
        self.security_config.security_mode = '1'  # BROWNFIELD
        self.security_config.save()
    ...
    def test_level_feature_map_dev_has_all_features(self):
        """Test LAB mode has the most features in LEVEL_FEATURE_MAP."""
        dev_features = LEVEL_FEATURE_MAP[SecurityConfig.SecurityModeChoices.LAB]

        # LAB should have all or most features
        for mode in SecurityConfig.SecurityModeChoices:
            mode_features = LEVEL_FEATURE_MAP[mode]
            # LAB should have at least as many features as any other mode
            # (or be a superset)
