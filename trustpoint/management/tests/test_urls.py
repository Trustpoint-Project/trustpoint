...
class SettingsUrlsTestCase(TestCase):
    ...
    def test_tls_url(self):
        ...
        url = reverse('management:tls')
        self.assertEqual(url, '/management/tls/')

        resolver = resolve(url)
        self.assertEqual(resolver.view_name, 'management:tls')
        self.assertEqual(resolver.func.view_class, tls.TlsView)

    def test_backups_url(self):
        """Test the backups management URL."""
        url = reverse('management:backups')
        self.assertEqual(url, '/management/backups/')

        resolver = resolve(url)
        self.assertEqual(resolver.view_name, 'management:backups')
    ...
    def test_backup_download_url(self):
        ...
        url = reverse('management:backup-download', kwargs={'filename': 'backup_20231201.tar.gz'})
        self.assertEqual(url, '/management/backups/download/backup_20231201.tar.gz/')

        resolver = resolve(url)
        self.assertEqual(resolver.view_name, 'management:backup-download')
    ...
    def test_backup_download_multiple_url(self):
        ...
        self.assertEqual(url, '/management/backups/download-multiple/tar.gz/')

        resolver = resolve(url)
        self.assertEqual(resolver.view_name, 'management:backup-download-multiple')
        ...
        self.assertEqual(url, '/management/backups/download-multiple/zip/')
    ...
    def test_backup_delete_multiple_url(self):
        ...
        self.assertEqual(url, '/management/backups/delete-multiple/')
    ...
    def test_logging_regex_patterns(self):
        ...
        for filename in valid_filenames:
            # Test details URL
            url = f'/management/logging/files/details/{filename}'
            resolver = resolve(url)
            self.assertEqual(resolver.view_name, 'management:logging-files-details')
            self.assertEqual(resolver.kwargs['filename'], filename)

            # Test download URL
            url = f'/management/logging/files/download/{filename}'
            resolver = resolve(url)
            self.assertEqual(resolver.view_name, 'management:logging-files-download')
            self.assertEqual(resolver.kwargs['filename'], filename)
