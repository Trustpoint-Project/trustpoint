...
if __name__ == '__main__':
    ...
    est_parser = EstMessageParser()
    est_authenticator = EstAuthentication()
    est_authorizer = EstAuthorization()

    try:
        est_validator.validate(est_request, est_context)
        est_parser.parse(est_request, est_context)
        est_authenticator.authenticate(est_request, est_context)
        est_authorizer.authorize(est_request, est_context)
