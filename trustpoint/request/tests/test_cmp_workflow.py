...
class TestCMPHelper(LoggerMixin):
    def test_cmp_no_onboarding_shared_secret_auth(self, cmp_device_without_onboarding) -> None:
        ...
        cmp_factory = (
            ...
            .add_component(
                CertificateRequest(
                    '/CN=Trustpoint-TlsServer-Credential/O=TestOrg/OU=TestOrgUnit',
                    10,
    ...
    def test_cmp_with_onboarding_client_certificate_auth(
        self, domain_credential_cmp_onboarding, rsa_private_key
    ) -> None:
        ...
        cmp_factory = (
            CompositeCMPCommand('test_cmp_onboarding', 'Test CMP onboarding command')
            .add_component(BasicCMPArgs(cmd=operation_str_short))
            .add_component(
                ServerConfig(
        ...
        assert (
            private_key.public_key().public_numbers() == mock_context.client_certificate.public_key().public_numbers()
    ...
    def test_unauthorized_device_access_using_cmp_mock_server(self, cmp_device_without_onboarding) -> None:
        ...
        cmp_factory = (
            ...
            .add_component(
                ServerConfig(
                    f'http://localhost:{free_port}/.well-known/cmp/certification/{domain_str}/{certtemplate_str}/'
                )
            )
            .add_component(SharedSecretAuth(f'{device.id}', f'pass:{invalid_shared_secret}'))
            .add_component(
                CertificateRequest(
        ...
        keygen_factory = (
            CompositeKeyGenerator('RSA').add_component(RSAKeyGenerator(2048)).add_component(KeyFileOutput())
        )
        ...
        try:
            ...
            assert path.startswith(f'/.well-known/cmp/certification/{domain_str}/{certtemplate_str}/'), (
            ...
            try:
                ...
                error_message = str(e)
                assert 'All authentication methods were unsuccessful' in error_message, (
