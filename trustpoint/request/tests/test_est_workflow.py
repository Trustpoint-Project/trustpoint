...
class TestESTHelper(LoggerMixin):
    def test_est_no_onboarding_username_password_auth(self, est_device_without_onboarding, rsa_private_key) -> None:
    ...
    def test_est_with_onboarding_client_certificate_authentication(
        self, domain_credential_est_onboarding, rsa_private_key
    ) -> None:
        ...
        assert mock_context.cert_requested is not None
        assert isinstance(mock_context.cert_requested, x509.CertificateSigningRequest), 'Request CSR is invalid'
        assert mock_context.domain == device.domain, 'Domain in context does not match expected domain'
