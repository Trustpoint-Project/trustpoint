...
class TestProfileValidator:
    ...
    def test_validate_invalid_profile_json(
        self,
        domain_instance: dict[str, Any]
    ) -> None:
    ...
    def test_validate_profile_validation_error(
        self,
        domain_instance: dict[str, Any]
    ) -> None:
        ...
        cert_profile = CertificateProfileModel.objects.create(
            unique_name='test-profile-strict',
    ...
    def test_validate_with_pydantic_validation_error(
        ...
        cert_profile = CertificateProfileModel.objects.create(
            unique_name='test-profile-pydantic',
        ...
        with patch('request.profile_validator.JSONProfileVerifier') as mock_verifier:
            ...
            validation_error = ValidationError.from_exception_data(
                'error',
                [{'type': 'value_error', 'loc': ('field',), 'msg': 'Invalid value', 'input': 'test', 'ctx': {'error': 'test'}}]
            )
    ...
    def test_validate_with_profile_validation_error_exception(
        self,
        domain_instance: dict[str, Any]
    ) -> None:
        ...
        cert_profile = CertificateProfileModel.objects.create(
            unique_name='test-profile-exception',
    ...
    def test_validate_logging(
        ...
        cert_profile = CertificateProfileModel.objects.create(
            unique_name='test-profile-logging',
            profile_json=json.dumps(profile_json)
        )        # Create a CSR
        builder = x509.CertificateSigningRequestBuilder()
        builder = builder.subject_name(x509.Name([
