...
from request.request_context import (
    BaseCertificateRequestContext, BaseCredentialRequestContext, BaseRequestContext, HttpBaseRequestContext
)
...
class LocalCaCertificateIssueProcessor(CertificateIssueProcessor):
    ...
    def process_operation(self, context: BaseRequestContext) -> None:  # noqa: C901, PLR0915 - Core pipeline orchestration requires multiple validation and conditional paths
        ...
        if isinstance(context, BaseCredentialRequestContext) and context.private_key:
            # standard issued credential with key pair generated by Trustpoint - save the private key along with the cert
            credential = CredentialSerializer(
                private_key=context.private_key.as_crypto(),
            ...
            context.issued_credential = saver._save(
                credential,
                common_name,
                credential_type,
