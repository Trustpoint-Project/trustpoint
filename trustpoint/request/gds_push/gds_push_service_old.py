    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            insecure: If True, skip credential and truststore setup for insecure operations.
                     Use this for server discovery without authentication.

        Raises:
            GdsPushError: If device configuration is invalid or CA chain cannot be built.
        """
        self.device = device
        ...
        self.domain_credential = self._get_domain_credential_from_device()
        if not self.domain_credential:
            msg = f'No domain credential found for device "{device.common_name}". Please issue a domain credential first.'
            raise GdsPushError(msg)

        # Build truststore from CA chain
        self.truststore = self._build_truststore_from_ca_chain()

        logger.info(
            'Initialized secure GDS Push service for device "%s" with %d CA certificate(s) in truststore',
            device.common_name,
            self.truststore.truststoreordermodel_set.count() if self.truststore else 0
        )

    def _get_domain_credential_from_device(self) -> IssuedCredentialModel | None:
        """Get the most recent domain credential from the device.

        Returns:
            The most recent domain credential, or None if not found
        """
        ...
        if domain_credentials.exists():
            credential = domain_credentials.first()
            logger.info(
                'Retrieved domain credential "%s" from device "%s"',
                credential.common_name,
                self.device.common_name
            )
            return credential
        
        logger.warning('No domain credential found for device "%s"', self.device.common_name)
        return None

    def _build_truststore_from_ca_chain(self) -> TruststoreModel:
        """Build truststore from the CA certificate chain (issuing CA to root CA).

        The truststore is built from the device's domain's issuing CA, following the
        parent_ca chain up to the root CA. This ensures the OPC UA server has all
        necessary certificates to validate client certificates.

        Returns:
            TruststoreModel: Temporary truststore containing the CA chain.

        Raises:
            GdsPushError: If device has no domain, domain has no issuing CA, or CA chain is incomplete.
        """
        from pki.models import TruststoreModel, TruststoreOrderModel

        # Check device has domain
        if not self.device.domain:
            msg = f'Device "{self.device.common_name}" has no domain configured'
            raise GdsPushError(msg)

        # Check domain has issuing CA
        ...
        # Build CA chain from issuing CA to root
        ca_chain = []
        current_ca = issuing_ca
        visited_cas = set()  # Prevent infinite loops in case of circular references
        ...
        while current_ca is not None:
            ...
            # Check if this is the root (self-signed)
        ...
        logger.info(
            'Built CA chain with %d certificate(s): %s',
            len(ca_chain),
            ' → '.join(ca.unique_name for ca in ca_chain)
        )
        
        # Create temporary truststore with unique name
        truststore_name = f'gds_push_{self.device.common_name}_{datetime.datetime.now().strftime("%Y%m%d_%H%M%S")}'
        truststore = TruststoreModel.objects.create(
            unique_name=truststore_name,
            description=f'Auto-generated truststore for GDS Push to device {self.device.common_name}'
        )
        
        # Add CA certificates to truststore in order (issuing CA first, root last)
        for order, ca in enumerate(ca_chain):
            cert_model = ca.ca_certificate_model
            TruststoreOrderModel.objects.create(
                truststore=truststore,
                certificate=cert_model,
                order=order
            )
            logger.debug('Added CA "%s" to truststore at order %d', ca.unique_name, order)
