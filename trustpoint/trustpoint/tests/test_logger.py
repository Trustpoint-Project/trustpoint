...
class TestLoggerMixin:
    ...
    def test_logger_different_classes_have_different_loggers(self, sample_class):
        ...
        assert logger_1 != logger_2, f'Loggers should be distinct for separate classes. Found the same: {logger_1.name}'
        assert logger_1.name != logger_2.name, (
            f'Logger names should differ for distinct classes. Found: {logger_1.name} and {logger_2.name}'
        )
