...
from settings.views import IndexView, backup, language, logging, pkcs11, security, tls
...
class SettingsUrlsTestCase(TestCase):
    ...
    def test_language_url(self):
        """Test the language settings URL."""
        url = reverse('settings:language')
        self.assertEqual(url, '/settings/language/')

        resolver = resolve(url)
        self.assertEqual(resolver.view_name, 'settings:language')
        self.assertEqual(resolver.func, language.language)
