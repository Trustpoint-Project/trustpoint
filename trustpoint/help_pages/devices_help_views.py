...
class NoOnboardingEstUsernamePasswordStrategy(HelpPageStrategy):
    ...
    def build_sections(self, help_context: HelpContext) -> tuple[list[HelpSection], str]:
        ...
        def _build_section(
            ...
            # Windows: install the resulting PEM certificate into the Windows
            # certificate store. This is deliberately kept as a single line
            # for cmd.exe friendliness.
            # Install the certificate into the local machine "My" store. This does not
            # associate the certificate with a Windows-managed private key (since the key
            # was generated by OpenSSL), but it makes the certificate available to
            # Windows tooling.
            windows_install_cmd = (
            ...
            return HelpSection(
                title,
                [
                    # Common OpenSSL flow (Linux & Windows)
                    HelpRow(
                        _non_lazy('Generate CSR with OpenSSL (Linux/Windows)'),
                        cmd,
                        ValueRenderType.CODE,
                    ),
                    HelpRow(
                        _non_lazy('Enroll certificate with curl (Linux/Windows)'),
                        value=curl_cmd,
                        value_render_type=ValueRenderType.CODE,
                    ),
...
def _build_windows_request_inf(subject_cn: str, dns_name: str | None = None) -> str:
    """Build a minimal Windows request.inf template for certreq.

    The template follows Microsoft's INF format and is intentionally conservative:
    - CN is taken from the device's common name.
    - A single DNS SubjectAltName is added (defaults to the CN).
    - EKU is fixed to serverAuth (1.3.6.1.5.5.7.3.1) for now.
    """
    ...
    # Use CRLF to better match Windows tooling expectations.
    return '\r\n'.join(lines)
...
def _build_inf_config_from_sample_request(
    sample_request: dict[str, Any],
    *,
    device_cn: str,
    domain_public_key_info: str | None,
) -> dict[str, Any]:
    """Build a lightweight internal config for the Windows request.inf file.

    This function derives a profile-aware subject DN and SAN from the
    sample_request generated by :class:`JSONProfileVerifier`.
    ...
    The resulting dict has the following structure::

        {
            ...
            "key_algorithm": "RSA" | "ECDSA_P256",
            "key_length": 2048 | 256,
        }
    """

    # Subject DN
    subject = sample_request.get('subject') or sample_request.get('subj') or {}

    def _get_subject_str(field: str) -> str | None:
        value = subject.get(field)
        if value is None:
            return None
        return str(value)

    subject_rdn: dict[str, str] = {}

    cn = _get_subject_str('common_name') or device_cn
    subject_rdn['CN'] = cn

    # Map common subject attributes to their Windows RDN abbreviations
    subject_mapping: list[tuple[str, str]] = [
        ('organization_name', 'O'),
    ...
    for field, rdn in subject_mapping:
        value = _get_subject_str(field)
        if value:
            subject_rdn[rdn] = value

    # SAN (Subject Alternative Name)
    extensions = sample_request.get('extensions') or sample_request.get('ext') or {}
    san = extensions.get('subject_alternative_name') or {}

    def _to_str_list(value: Any) -> list[str]:
        """Normalize SAN values to a list of strings."""
        if value is None:
            return []
    ...
    def _filter_placeholders(values: list[str]) -> list[str]:
        """Filter out CHANGEME-style placeholders from profile SAN values.

        Many JSON profiles use markers such as ``CHANGEME_ip_addresses_required`` to
        express that an operator must provide a concrete value. Those placeholders
        are not valid SAN values for Windows ``certreq`` and would cause
        ERROR_INVALID_PARAMETER if emitted into the INF file.
        """

        return [v for v in values if not str(v).upper().startswith('CHANGEME_')]

    san_dns = _filter_placeholders(_to_str_list(san.get('dns_names')))
    san_ip = _filter_placeholders(_to_str_list(san.get('ip_addresses')))
    san_email = _filter_placeholders(_to_str_list(san.get('rfc822_names')))
    san_uri = _filter_placeholders(_to_str_list(san.get('uris')))

    # If the profile doesn't define any DNS SANs, use CN as a pragmatic default
    if not san_dns and cn:
        san_dns = [cn]

    # Key algorithm selection based on domain public key info
    pk_info = (domain_public_key_info or '').lower()
    if 'ec' in pk_info or 'secp256r1' in pk_info or 'ecdsa' in pk_info:
...
def _build_windows_request_inf_from_config(config: dict[str, Any]) -> str:
    """Render a Windows request.inf file from the internal config.

    The result is suitable for use with ``certreq -new`` on Windows.
    """

    subject_rdn: dict[str, str] = config.get('subject_rdn', {})
    subject_parts = [f'{k}={v}' for k, v in subject_rdn.items() if v]
    subject_str = ', '.join(subject_parts) if subject_parts else ''
    ...
    lines: list[str] = [
        ...
        'MachineKeySet = TRUE',
        f'KeyAlgorithm = {config.get("key_algorithm", "RSA")}',
        f'KeyLength = {config.get("key_length", 2048)}',
        'HashAlgorithm = SHA256',
        'Exportable = FALSE',
        'RequestType = PKCS10',
        'ProviderName = "Microsoft Software Key Storage Provider"',
    ...
    san_parts: list[str] = []
    for dns in config.get('san_dns', []) or []:
        san_parts.append(f'DNS={dns}')
    for ip in config.get('san_ip', []) or []:
        san_parts.append(f'IPAddress={ip}')
    for email in config.get('san_email', []) or []:
        san_parts.append(f'email={email}')
    for uri in config.get('san_uri', []) or []:
        san_parts.append(f'URL={uri}')

    if san_parts:
        lines.append('[Extensions]')
        lines.append('2.5.29.17 = "{text}"')
        for idx, part in enumerate(san_parts):
            prefix = '' if idx == 0 else '&'
            lines.append(f'_continue_ = "{prefix}{part}"')
        lines.append('')

    # CRLF newlines to match typical Windows INF expectations
    return '\r\n'.join(lines)


class EstUsernamePasswordRequestInfDownloadView(PageContextMixin, View):
    """Serve a dynamically generated Windows request.inf for EST username/password enrollment.

    The INF file is tailored to the selected device and certificate profile but intentionally keeps
    the structure minimal and conservative so that users can adjust details if needed.
    """
    ...
    def get(self, request: Any, pk: int, profile_name: str, *args: Any, **kwargs: Any) -> HttpResponse:  # noqa: D401
        """Return the INF file as a text/plain attachment."""
        del args, kwargs

        device = DeviceModel.objects.filter(pk=pk).select_related('domain').first()
        if not device:
            raise Http404(_('Device not found.'))

        no_onboarding_config = getattr(device, 'no_onboarding_config', None)
        if not no_onboarding_config or not no_onboarding_config.est_password:
            raise Http404(_('EST username/password is not configured for this device.'))
        ...
        allowed_app_profiles = list(
            domain.get_allowed_cert_profiles(
        ...
        profile = next(
            (
                p for p in allowed_app_profiles
                if (p.alias or p.certificate_profile.unique_name) == profile_name
            ),
            None,
        )
        if not profile:
            raise Http404(_('Certificate profile not allowed for this device.'))

        subject_cn = device.common_name or device.name or str(device.pk)

        # Build a profile-aware INF file using JSONProfileVerifier's sample_request.
        # If anything goes wrong (e.g. unexpected profile structure), fall back to
        # the previous minimal INF generator to keep behavior backwards compatible.
        try:
            from pki.util.cert_profile import JSONProfileVerifier  # local import to avoid circular deps

            cert_profile_dict = profile.certificate_profile.profile
            verifier = JSONProfileVerifier(cert_profile_dict)
            sample_request = verifier.get_sample_request()

            inf_config = _build_inf_config_from_sample_request(
                sample_request,
                device_cn=subject_cn,
                domain_public_key_info=str(getattr(domain, 'public_key_info', '') or ''),
            )
            content = _build_windows_request_inf_from_config(inf_config)
        except Exception:  # noqa: BLE001
            dns_name = subject_cn
            content = _build_windows_request_inf(subject_cn=subject_cn, dns_name=dns_name)

        response = HttpResponse(content, content_type='text/plain; charset=utf-8')
        response['Content-Disposition'] = 'attachment; filename="request.inf"'
        return response
