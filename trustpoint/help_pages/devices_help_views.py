...
class BaseHelpView(PageContextMixin, DetailView[DeviceModel]):
    ...
    def _make_context(self, host_ip: str = '127.0.0.1') -> HelpContext:
        ...
        host_base = f'https://{host_ip}:{self.request.META.get("HTTP_X_FORWARDED_PORT", settings.ADVERTISED_PORT)}'
...
"""Windows request.inf generation helpers removed.

Historically, this module exposed helpers to generate a Windows request.inf
file and a corresponding download view. The UI now guides users exclusively
via OpenSSL/curl/certutil command snippets instead of downloading an INF
template that still required manual edits. The old helpers were therefore
