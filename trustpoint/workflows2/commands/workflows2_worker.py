"""Run the Workflow 2 background worker."""
...
class Command(BaseCommand):
    """Consume queued Workflow 2 jobs from the database-backed worker queue."""

    help = 'Run workflows2 worker (consumes Workflow2Job queue).'

    def add_arguments(self, parser: Any) -> None:
        """Register command-line arguments for the worker command."""
    ...
    def _wait_for_tables(self, *, tables: list[str], timeout_seconds: int) -> None:
        """Wait until the required workflow tables are available."""
        start = time.time()
        missing = set(tables)
        ...
        while missing:
            ...
            if timeout_seconds and (time.time() - start) >= timeout_seconds:
                error_message = f'Timed out waiting for migrations/tables: {sorted(missing)}'
                raise RuntimeError(error_message)

            time.sleep(1.0)

    def handle(self, *_args: Any, **opts: Any) -> None:
        """Start the worker loop and keep polling until stopped."""
        once = bool(opts['once'])
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f'[workflows2_worker] starting id={worker_id} '
                f'lease={lease_seconds}s batch={batch_limit} sleep={sleep_seconds}s'
            )
        )
