...
class WorkflowCompiler:
    ...
    def _compile_webhook_params(step: dict[str, Any], base: str) -> dict[str, Any]:
        ...
        # ----------------------------
        # Capture (NEW ONLY): vars.<name> -> source
        # Example:
        #   capture:
        #     vars.http_status: status_code
        #     vars.req_id: headers.x-request-id
        #     vars.some_value: body.some_value
        #
        # Compiled IR:
        #   capture: [{"target": ["vars","http_status"], "source": ["status_code"]}, ...]
        # ----------------------------
        ...
        def _parse_vars_target(key: Any, *, path: str) -> list[str]:
            ...
            parts = key.split(".")
            # keep vars flat (current runtime model)
        ...
        def _parse_source(val: Any, *, path: str) -> list[str]:
            ...
            if s.startswith("headers."):
                ...
                # header names may contain dashes => keep as one segment (no further split)
                return ["headers", rest]
            ...
            if s.startswith("body."):
                ...
                # allow dotted traversal for JSON bodies
    ...
    def _validate_flow_completeness(self, steps_ir: dict[str, Any], transitions: dict[str, Any]) -> None:
        for step_id, s in steps_ir.items():
            ...
            if produces:
                ...
                # Non-outcome steps may omit transitions -> implicit $end
                if tr is None:
                    continue
