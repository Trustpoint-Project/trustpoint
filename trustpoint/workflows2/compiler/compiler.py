...
_ALLOWED_COMPUTE_OPS: set[str] = {
    ...
    'int',
    'float',
    # optional extras (only if your eval supports them)
    # "mod",
}
...
class WorkflowCompiler:
    ...
    TERMINAL_TYPES: set[str] = {'stop', 'succeed', 'fail', 'reject'}
    ...
    def _compile_step_params(self, step_id: str, typ: str, step: dict[str, Any]) -> dict[str, Any]:
        ...
        if typ == 'reject':
            return self._compile_reject_params(step, base)
        if typ == 'stop':
    ...
    def _compile_compute_params(step: dict[str, Any], base: str) -> dict[str, Any]:
        """Supports BOTH:
          A) vars.x: ${add(vars.a, 1)}
          B) vars.x:
               add: ["${vars.a}", 1]
        Normalizes to: {"set": {"vars.x": {"kind":"expr","expr": <expr_ir>}}}
        """
        from .expr import parse_required_expr_string
        from .templates import _expr_to_ir  # existing helper
    ...
    def _compile_compute_yaml_op(node: dict[str, Any], *, path: str) -> dict[str, Any]:
        """Converts YAML op tree -> expr IR (call/lit/ref/template).

        Example:
          {"add": ["${vars.a}", 10]} -> {"kind":"call","name":"add","args":[...]}
        """
        ...
        op, args = next(iter(node.items()))  # <-- op is DEFINITELY assigned here
    ...
    def _compile_compute_value(v: Any, *, path: str) -> dict[str, Any]:
        """Value nodes for compute ops:
        - literals (int/float/bool/None/str)
        - expression strings like "${vars.a}" or "${add(...)}"
        - nested op dicts like {"mul":[...]}
        """
        # nested op
        if isinstance(v, dict):
            return WorkflowCompiler._compile_compute_yaml_op(v, path=path)

        # expression string (must be a *full* expr string)
        if isinstance(v, str):
            s = v.strip()
    ...
    @staticmethod
    def _compile_reject_params(step: dict[str, Any], base: str) -> dict[str, Any]:
        reason = step.get('reason')
        if not isinstance(reason, str) or not reason.strip():
            raise CompileError('"reject.reason" must be a non-empty string', path=f'{base}.reason')
