# workflows2/compiler/templates.py
...
def compile_templates_deep(obj: Any, *, path: str) -> Any:
    """
    Recursively compile `${...}` templates in:
      - strings
      - lists
    ...
    if isinstance(obj, dict):
        ...
        for k, v in obj.items():
            if not isinstance(k, str):
                raise CompileError("Only string keys are supported in mappings.", path=path)
...
def _expr_to_ir(expr_ast: Any) -> Any:
    ...
    raise CompileError("Unsupported expression AST node")
